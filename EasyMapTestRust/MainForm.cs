...
namespace EasyMapTestRust
{
	...
	public partial class MainForm : Form
	{
		...
		private void MainForm_Load(object sender, EventArgs e)
		{
			...
			NextStepBrowserHelp.DocumentText = @"<!DOCTYPE html><html lang=""en""><head><meta charset=""UTF-8""><meta name=""viewport"" content=""width=device-width, initial-scale=1.0""><title>DropLaunch Server Instructions</title><style>body{font-family:sans-serif;line-height:1.6;margin:20px;background-color:#c8c8c8;}h2{color:#2c3e50;border-bottom:2px solid #3498db;padding-bottom:5px;}p{margin-bottom:10px;}code{background-color:#ecf0f1;padding:2px 5px;border-radius:3px;font-family:""Courier New"", Courier, monospace;}.note{background-color:#f9f9f9;border-left:5px solid #f1c40f;padding:10px;margin-top:15px;}ol{padding-left:20px;}li{margin-bottom:8px;}</style></head><body><h2>Connecting to Your Server</h2><p>After launching your server with DropLaunch, you can connect in-game.</p><ol><li>If enabled in the setup, the connection command <code>connect localhost:28015</code> is automatically copied to your clipboard.</li><li>In the game's main menu, press <strong>F1</strong> to open the console.</li><li>Paste or type the command <code>connect localhost:28015</code> and press <strong>Enter</strong>.</li></ol><hr><h2>Becoming a Server Admin</h2><p>To grant yourself administrative privileges on the server, follow these steps:</p><ol><li><strong>Find Your SteamID64:</strong><p>You can easily find your SteamID64 by using a website like <a href=""https://steamid.io/"" target=""_blank"">steamid.io</a>.</p></li><li><strong>Assign Ownership:</strong><p>In your running server console (the black window), type the command <code>ownerid YOUR_STEAMID64</code>. Be sure to replace <code>YOUR_STEAMID64</code> with your actual SteamID64.</p><p><em>Example:</em> <code>ownerid 76561198000000000</code></p><p>The console will confirm that you have been added as an owner.</p></li><li><strong>Save Changes:</strong><p>To make your admin status permanent, run the command <code>writecfg</code> in the server console. This will save the changes to the server's configuration files.</p></li></ol></body></html>";
            PrefabStartWebbrowser.DocumentText = @"<!DOCTYPE html><html lang=""en""><head><meta charset=""UTF-8""><meta name=""viewport"" content=""width=device-width, initial-scale=1.0""><title>Prefab Testing Start File</title><style>body {font-family: sans-serif;line-height: 1.6;margin: 20px;background-color: #c8c8c8;}h2 {color: #2c3e50;border-bottom: 2px solid #3498db;padding-bottom: 5px;}p, li {color: #34495e;}ul {list-style-type: none;padding-left: 0;}li {background-color: #ecf0f1;margin-bottom: 10px;padding: 10px;border-left: 5px solid #3498db;}code {font-family: ""Courier New"", Courier, monospace;font-weight: bold;color: #d35400;}</style></head><body><h2>Prefab & Map Testing Start File</h2><p>The ""Create Prefab Start File and Start Server"" option is designed for more focused, isolated testing. This is especially useful when you are working on a specific monument, puzzle, or custom prefab. This start file adds several commands to the server launch to allow map testing of unfinished maps.</p><ul><li><code>+cargoship.event_enabled False</code><br>Disables the Cargo Ship event.</li><li><code>+baseboat.generate_paths False</code><br>Stops boats from generating paths.</li><li><code>+ai.ocean_patrol_path_iterations 0</code><br>Disables AI pathing for ocean patrols.</li><li><code>+server.events False</code><br>Turns off most random server events.</li><li><code>+antihack.terrain_protection 0</code><br>Disables some anti-hacking measures related to terrain, which can be useful when testing building mechanics or custom terrain features.</li></ul><p>It’s perfect for ensuring a custom monument works correctly or for fine-tuning a puzzle without interference from normal game events.</p></body></html>";
			FileWatcherWebbrowser.DocumentText = @"<!DOCTYPE html><html lang=""en""><head><meta charset=""UTF-8""><meta name=""viewport"" content=""width=device-width, initial-scale=1.0""><title>File Watcher Details</title><style>body{font-family:sans-serif;line-height:1.6;margin:20px;background-color:#c8c8c8;}h2{color:#2c3e50;border-bottom:2px solid #3498db;padding-bottom:5px;}p,li{color:#34495e;}ul{padding-left:20px;}li{margin-bottom:10px;}</style></head><body><h2> File Watcher Feature</h2><p>The file watcher will automatically keep an eye on your map files inside your active folder.</p><ul><li><strong>Real-Time Monitoring:</strong> Monitors the map directory you've specified. It looks for any changes to files with a .map extension.</li><li><strong>Instant Updates:</strong> If you create, delete, rename, or save a .map file in the monitored folder, the file watcher detects this change and instantly updates the list of maps you see in the ""Active Folder"" tab.</li><li><strong>Desktop Notifications:</strong> If you have desktop notifications enabled, the file watcher will also send you a Windows notification whenever a map file is changed.</li><li><strong>Quick Launch:</strong> You can click directly on the notification to launch a server with the newly saved map, making your testing workflow even faster.</li></ul><p>You can enable or disable the file watcher and desktop notifications from the Settings page in the application.</p></body></html>";
